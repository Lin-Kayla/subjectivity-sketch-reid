...
from data_loader import TestData, PKUDataAttr
...
parser.add_argument('--dataset', default='pku', help=' dataset name: mask1k or pku]')
parser.add_argument('--data_path', default='/data3/lkj/rebuttal_sketchreid/dataset/pku/', type=str, help='path to dataset')
...
log_path = os.path.join(args.log_path, 'pku')
...
sys.stdout = Logger(os.path.join(log_path, suffix + '_os.txt'))
...
# training set
trainset = PKUDataAttr(data_path, transform=transform_train)
# generate the idx of each person identity
color_pos, thermal_pos = GenIdx(trainset.train_color_label, trainset.train_sketch_label)

# testing set
query_img, query_label = process_test_pku(data_path, modal='thermal')
...
print('  thermal  | {:5d} | {:8d}'.format(n_class, len(trainset.train_sketch_label)))
print('  ------------------------------')
...
net = model(n_class, model_clip, args.batch_size, args.num_pos, arch=args.arch, train_multi_query=False, test_multi_query=False)
net.to(device)
...
def test(epoch):
    ...
    with torch.no_grad():
        for batch_idx, (input, label) in enumerate(gall_loader):
            ...
            feat = net(input, input, None, None, test_mode[0])['feat4_p_norm']
    ...
    with torch.no_grad():
        for batch_idx, (input, label) in enumerate(query_loader):
            ...
            if torch.cuda.is_available():
                ...
                input = Variable(input)
            feat = net(input, input, None, None, test_mode[1])['feat4_p_norm']
